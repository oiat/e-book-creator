...
class qqFileUploader {
    ...
    function handleUpload($uploadDirectory, $replaceOldFile = FALSE){
		...
		// get $uploadDirectory
		$q = "SELECT a.uid FROM " . TBL_BOOKS . " as a, " . TBL_BOOKS_STRUCTURE . " as b, " . TBL_BOOKS_WIDGETS . " as c  WHERE c.id='$widget_id' and c.chapter_id = b.id and b.book_id = a.id";
        ...
        if ($this->file->save($uploadDirectory . $widget_id . '.' . $ext)){
			$fsave = '' . $widget_id . '.' . $ext . '';
			
			$forig = $uploadDirectory . $widget_id . '.' . $ext;
			// resample file
			...
			$q = "UPDATE " . TBL_BOOKS_WIDGETS . " set image='$fsave' WHERE id='$widget_id'";
			$result = mysql_query($q, $this->_db->connection); 
			return array("success"=>true,"id"=>"$widget_id","uid"=>"$uid","image"=>"$fsave");
        } else {
